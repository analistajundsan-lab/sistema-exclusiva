...
import {
  ...
  Camera, FileText, Wifi, CheckCircle2, ImagePlus, X, Bus, ShieldCheck,
} from 'lucide-react'
...
const CAMERAS = [
  ...
  { key: 'camera_salao', label: 'Salao' },
] as const

const LIC_OPTIONS = [
  { value: 'SIM_EM_DIA', label: 'Sim, em dia' },
  { value: 'NAO_IMPRIMIR_NOVAMENTE', label: 'Nao -- imprimir novamente' },
...
const CHECKLIST_OPTIONS = [
  ...
  { value: 'NAO_MANUTENCAO_FORA_GARAGEM', label: 'Nao -- veiculo em manutencao ou fora da garagem' },
  { value: 'JA_POSSUI_CHECKLIST_MES', label: 'Ja possui checklist do mes' },
...
const ARTESP_OPTIONS = [
  ...
  { value: 'NAO_COLOCAR_NOVO', label: 'Nao -- colocar novo' },
  { value: 'NA', label: 'N/A' },
]

const WIFI_OPTIONS = [
  { value: 'SIM_FUNCIONAL', label: 'Sim, funcional' },
  { value: 'NAO_SEM_REDE', label: 'Nao -- conectado porem sem rede' },
  { value: 'NAO_APARECE_LISTA', label: 'Nao aparece na lista de Wi-Fi' },
  { value: 'NAO_FUNCIONA_FRETADAO', label: 'Nao funciona no app Fretadao' },
]

// Conferencia de Documentos
const CRLV_EMTU_OPTIONS = [
  { value: 'VALIDO', label: 'Valido -- em dia' },
  { value: 'VENCIDO', label: 'Vencido' },
  { value: 'AUSENTE', label: 'Ausente no veiculo' },
  { value: 'ILEGIVEL', label: 'Ilegivel / danificado' },
]

const ARTESP_DOC_OPTIONS = [
  { value: 'PRESENTE_VALIDO', label: 'Presente e valido' },
  { value: 'PRESENTE_VENCIDO', label: 'Presente e vencido' },
  { value: 'AUSENTE', label: 'Ausente no veiculo' },
  { value: 'NA', label: 'N/A' },
]

const EMDEC_DOC_OPTIONS = [
  { value: 'PRESENTE_VALIDO', label: 'Presente e valido' },
  { value: 'PRESENTE_VENCIDO', label: 'Presente e vencido' },
  { value: 'AUSENTE', label: 'Ausente no veiculo' },
  { value: 'NA', label: 'N/A' },
...
const UNRESTRICTED_ROLES = ['admin', 'gerente', 'supervisao', 'supervisor']
...
interface Form {
  ...
  crlv_emtu: string; crlv_emtu_qrcode: boolean | undefined
  artesp_doc: string; emdec_doc: string
...
const INITIAL: Form = {
  ...
  crlv_emtu: '', crlv_emtu_qrcode: undefined,
  artesp_doc: '', emdec_doc: '',
      ...
      {options.map(opt => (
        ...
        >
          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
            selected === opt.value ? 'border-brand-700 bg-brand-700' : 'border-gray-300 dark:border-gray-600'
          }`}>
            {selected === opt.value && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
          </div>
          <span className={`text-sm font-medium ${selected === opt.value ? 'text-brand-800 dark:text-brand-300' : 'text-gray-700 dark:text-gray-300'}`}>
            {opt.label}
          </span>
        </button>
      ))}
...
export function ChecklistNovo() {
  ...
  useEffect(() => {
    if (isUnrestricted) {
      api.get<string[]>('/checklist/garagens')
        .then(res => setAllGaragens(res.data))
        .catch(() => { /* silencioso */ })
    }
  }, [isUnrestricted])
  ...
  // AVULSO: 0 -> 3 -> 4
  // MENSAL: 0 -> 1 -> 2 -> 5 -> 3 -> 4
  const steps = form.tipo === 'AVULSO' ? [0, 3, 4] : [0, 1, 2, 5, 3, 4]
  ...
  const handlePhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (form.evidencias.length + files.length > 2) {
      alert('Maximo de 2 fotos por checklist.')
      return
    }
    setImgLoading(true)
  ...
  const STEP_ICONS: Record<number, React.ElementType> = {
    0: Bus, 1: Camera, 2: FileText, 3: Wifi, 4: CheckCircle2, 5: ShieldCheck,
  }
      ...
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6 text-sm">
            Veiculo <strong>{form.prefixo.toUpperCase()}</strong> -- {form.garagem}
          </p>
      ...
      <div className="space-y-4 mb-8">
        ...
        {step === 0 && (
          <>
              ...
              {isUnrestricted ? (
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    value={form.garagem}
                    onChange={e => set('garagem', e.target.value)}
                    placeholder="Ou digite a garagem manualmente"
                    className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm font-medium placeholder:font-normal placeholder:text-gray-400"
                  />
                </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Prefixo do veiculo *</label>
              <input
                type="text"
                value={form.prefixo}
        ...
        {/* STEP 1: Cameras + Acessorios */}
        {step === 1 && (
          <>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Cameras</p>
            {CAMERAS.map(cam => {
            ...
            <div className="mt-2 space-y-3">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide pt-1">Acessorios</p>
              <div>
        ...
        {step === 2 && (
          <>
            ...
            <div>
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Cartao ARTESP</p>
              <RadioCard options={ARTESP_OPTIONS} selected={form.cartao_artesp} onChange={v => set('cartao_artesp', v)} />
            </div>

            <div>
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3 pt-1">Materiais Graficos</p>
              {[
        ...
        {/* STEP 5: Conferencia de Documentos (CRLV-EMTU, ARTESP, EMDEC) */}
        {step === 5 && (
          <>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">
              Conferencia de Documentos Obrigatorios
            </p>

            {/* CRLV-EMTU */}
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl px-4 py-3 space-y-3">
              <p className="text-sm font-bold text-gray-700 dark:text-gray-200">CRLV / EMTU</p>
              <RadioCard options={CRLV_EMTU_OPTIONS} selected={form.crlv_emtu} onChange={v => set('crlv_emtu', v)} />
              <div>
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">QR Code EMTU legivel / valido?</p>
                <YesNo value={form.crlv_emtu_qrcode} onChange={v => set('crlv_emtu_qrcode', v)} />
              </div>
            </div>

            {/* ARTESP */}
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl px-4 py-3 space-y-2">
              <p className="text-sm font-bold text-gray-700 dark:text-gray-200">Documento ARTESP</p>
              <RadioCard options={ARTESP_DOC_OPTIONS} selected={form.artesp_doc} onChange={v => set('artesp_doc', v)} />
            </div>

            {/* EMDEC */}
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl px-4 py-3 space-y-2">
              <p className="text-sm font-bold text-gray-700 dark:text-gray-200">Documento EMDEC</p>
              <RadioCard options={EMDEC_DOC_OPTIONS} selected={form.emdec_doc} onChange={v => set('emdec_doc', v)} />
            </div>
          </>
        )}
        ...
        {step === 3 && (
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Wi-Fi esta funcional?</p>
          ...
          <>
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                Observacoes <span className="font-normal text-gray-400">(opcional)</span>
              </label>
              <textarea
                value={form.observacoes}
                onChange={e => set('observacoes', e.target.value)}
                placeholder="Descreva qualquer nao-conformidade encontrada no veiculo..."
                rows={4}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                Evidencias <span className="font-normal text-gray-400">(fotos -- opcional, max. 2)</span>
              </label>
